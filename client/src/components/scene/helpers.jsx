...
export function DeckBoards({ deckColor, base, shape, W = 900, DECK_Y = 342 }) {
  const b = deckColor?.h || base || '#9A7E5C';
  const img = deckColor?.img;
  ...
  return (
    <g>
      <defs>
        ...
        {img && (
          <pattern id="p-deck-img" patternUnits="userSpaceOnUse" width="200" height="200">
            <image href={img} width="200" height="200" preserveAspectRatio="xMidYMid slice" />
          </pattern>
        )}
      </defs>
      ...
      <g clipPath="url(#cp-deck-main)">
        <rect x={0} y={DECK_Y - 12} width={W} height={e.depth + 24} fill={img ? "url(#p-deck-img)" : b} filter={img ? undefined : "url(#f-wood)"} />
      ...
      {e.upper && (
        <g clipPath="url(#cp-deck-upper)">
          <rect x={0} y={e.upperY - 12} width={W} height={e.upperH + 24} fill={img ? "url(#p-deck-img)" : lighten(b, 0.06)} filter={img ? undefined : "url(#f-wood)"} />
