  ...
  return (
    <svg viewBox="0 0 900 520" style={{ width: '100%', height: '100%', display: 'block' }}>
      <defs>
        ...
        {deckColor?.img && (
          <pattern id="p-detail-img" patternUnits="userSpaceOnUse" width="300" height="300">
            <image href={deckColor.img} width="300" height="300" preserveAspectRatio="xMidYMid slice" />
          </pattern>
        )}
      </defs>
        ...
        {Array.from({ length: numBoards }, (_, bi) => {
          ...
          return (
            <g key={bi}>
              ...
              <rect x={x} y={0} width={bW} height={SH} fill={deckColor?.img ? "url(#p-detail-img)" : bColor} filter={deckColor?.img ? undefined : "url(#dv-wood-grain)"} />
              {deckColor?.img && <rect x={x} y={0} width={bW} height={SH} fill={bi % 7 === 0 ? "rgba(0,0,0,0.12)" : bi % 7 === 3 ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.04)"} />}
              {/* Sheen overlay */}
      ...
      {c && (
        <g>
          ...
          <rect x="733" y="463" width="28" height="34" rx="4" fill={deckColor?.img ? "url(#p-detail-img)" : c}
