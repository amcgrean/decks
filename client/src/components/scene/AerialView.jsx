  ...
  return (
    <svg viewBox={`0 0 ${W} 520`} style={{ width: '100%', height: '100%', display: 'block' }}>
      <defs>
        ...
        {deckColor?.img && (
          <pattern id="p-aerial-img" patternUnits="userSpaceOnUse" width="150" height="150">
            <image href={deckColor.img} width="150" height="150" preserveAspectRatio="xMidYMid slice" />
          </pattern>
        )}
      ...
      {decks.map(([dx, dy, dw, dh], si) => (
        <g key={si} filter="url(#f-deck-top-shadow)">
          <rect x={dx} y={dy} width={dw} height={dh} fill={deckColor?.img ? "url(#p-aerial-img)" : b} filter={deckColor?.img ? undefined : "url(#f-wood-top)"} />
          {Array.from({ length: Math.ceil(dh / 10) + 1 }, (_, i) => (
            <rect key={i} x={dx} y={dy + i * 10} width={dw} height={9}
              fill={deckColor?.img ? "url(#p-aerial-img)" : (i % 3 === 0 ? lighten(b, 0.1) : i % 3 === 1 ? b : darken(b, 0.04))} />
          ))}
          {deckColor?.img && Array.from({ length: Math.ceil(dh / 10) + 1 }, (_, i) => (
            <rect key={i} x={dx} y={dy + i * 10} width={dw} height={9} fill={i % 3 === 0 ? "rgba(255,255,255,0.08)" : i % 3 === 1 ? "transparent" : "rgba(0,0,0,0.06)"} />
          ))}
          {Array.from({ length: Math.ceil(dh / 10) + 1 }, (_, i) => (
